...
export function createRevision(shell: (html: string, testing: boolean) => void) {
  ...
  function render(html: string, testing = false) {
    ...
    study.updateHeader();
    button('rev-reveal', revealFrench);
    document.querySelectorAll<HTMLButtonElement>('[data-rev-page]').forEach(b => b.onclick = () => {
      if (!saving) lesson(Number(b.dataset.revPage));
    });
    button('rev-header-test', () => { if (!saving && !active) openTest(); });
  ...
  function overview() {
    study.stop();
    active = false; themeOpen = false; testInProgress = false; lessonIndex = null;
    ...
    render(`<div class="rev-eyebrow">LIRE · COMPRENDRE · RETENIR</div><h1 tabindex="-1">Révision<span class="rev-title-dot">.</span></h1><p class="rev-intro">Une idée à la fois.<br>Le code en français, les réflexes en catalan.</p>
      ...
      <div class="rev-note"><b>Comment est calculée ta maîtrise ?</b><p>La moyenne des <strong>5 derniers tests terminés</strong> de chaque thème. Au début, on utilise les tests disponibles. Un test quitté ne compte pas dans la maîtrise ; son temps reste enregistré.</p><p>Chaque question vaut 1 point. Aucune limite de temps. Un clic valide ta réponse et révèle la correction en français. Un second clic permet de continuer. Le drapeau 🇫🇷 dévoile les traductions si tu en as besoin avant de répondre.</p><p>Le chrono démarre à l’ouverture d’une fiche et cumule lecture, tests et corrections jusqu’au retour aux thèmes. Il se met en pause quand l’app est masquée. Les temps antérieurs à cette fonction ne sont pas disponibles.</p></div>
  ...
  function lesson(pageIndex: number) {
    ...
    render(`${back}<div class="rev-lesson rev-${theme.color}"><div class="rev-eyebrow">${theme.icon} ${esc(theme.title)} · Fiche ${pageIndex + 1} / ${theme.pages.length}</div><h1 tabindex="-1">${esc(p.title)}</h1><p class="rev-kicker">${esc(p.kicker)}</p>
      ...
      <div class="rev-ready"><span>${testInProgress ? 'Tes réponses sont conservées pendant ta lecture.' : pageIndex < theme.pages.length - 1 ? 'Continue la lecture à ton rythme.' : 'Tu te sens prêt ? À toi de jouer.'}</span><button class="rev-primary" id="rev-continue">${testInProgress ? 'Reprendre le test →' : pageIndex < theme.pages.length - 1 ? `Lire la fiche ${pageIndex + 2} →` : 'Lancer le test · 10 questions →'}</button><p>Les numéros du header ouvrent les fiches ; Q ${testInProgress ? 'reprend ton test' : 'ouvre les questions'}.</p></div>${history()}${sources()}</div>`);
    button('rev-home', overview);
    button('rev-continue', () => testInProgress ? openTest() : pageIndex < theme.pages.length - 1 ? lesson(pageIndex + 1) : start());
  }
  function openTest() {
    if (!testInProgress) { start(); return; }
    active = true; lessonIndex = null;
    question();
  }
  function start() {
    study.start(theme.id);
    ...
    picks = []; index = 0; active = true; testInProgress = true; lessonIndex = null; prepareQuestion();
  }
  ...
  const canAdvance = (target: EventTarget | null) => active && selected !== undefined && !saving
    ...
    && !(target instanceof Element && target.closest('header, #rev-quit, .rev-dialog-backdrop, a, input, textarea, select, summary'));
  ...
  function showFeedback() {
    ...
    feedback.innerHTML = `<strong>${correct ? '✓ Bonne réponse !' : pick === null ? '↷ Question passée' : '✕ Mauvaise réponse'}</strong><p>Bonne réponse : <b>${String.fromCharCode(65 + order.indexOf(q.correct))}</b> · ${esc(q.french.answers[q.correct])}</p><p>${esc(q.explanation)}</p>`;
    const hint = document.querySelector<HTMLElement>('#rev-advance-hint')!;
    hint.hidden = false;
    hint.textContent = index === TEST_LENGTH - 1 ? 'Clique à nouveau n’importe où pour voir ton bilan.' : 'Clique à nouveau n’importe où pour continuer.';
  ...
  function question() {
    ...
    render(`<div class="rev-test-top"><button id="rev-quit" class="rev-back">← Quitter</button><span>Question <b>${index + 1} / ${TEST_LENGTH}</b></span></div><div class="rev-progress" role="progressbar" aria-label="Questions terminées" aria-valuenow="${index}" aria-valuemin="0" aria-valuemax="${TEST_LENGTH}"><span style="width:${index * 10}%"></span></div>
      <div class="rev-eyebrow">${theme.icon} ${esc(theme.title)}</div><p class="rev-test-hint">Clique sur une réponse pour la valider.</p><h1 class="rev-question" lang="ca" tabindex="-1">${esc(q.prompt)}</h1><p id="rev-question-fr" class="rev-question-fr concealed" lang="fr" aria-hidden="true">${esc(q.french.prompt)}</p>
    ...
    button('rev-quit', () => {
      ...
      controls[0].focus();
      dialog.onkeydown = e => {
        if (e.key === 'Escape') { area.innerHTML = ''; document.querySelector<HTMLButtonElement>('#rev-quit')?.focus(); }
        if (e.key === 'Tab') { e.preventDefault(); controls[document.activeElement === controls[0] ? 1 : 0].focus(); }
      };
      button('rev-resume', () => { area.innerHTML = ''; document.querySelector<HTMLButtonElement>('#rev-quit')?.focus(); });
      button('rev-abandon', () => { active = false; testInProgress = false; questions = []; lesson(0); });
    });
    if (frenchVisible) revealFrench();
    if (selected !== undefined) showFeedback();
  }
  function advance() {
    if (!active || selected === undefined || saving) return;
    if (++index < questions.length) prepareQuestion();
    else {
      active = false; testInProgress = false;
      const run = finishRun(theme.id, questions, picks);
      runs = [...runs, run];
  ...
  function result(run: RevisionRun) {
    const score = Math.round(100 * run.correct / run.total);
    const missed = questions.filter((q, i) => picks[i] !== q.correct).length;
    render(`${back}<div class="rev-result"><div class="rev-eyebrow">${theme.icon} ${esc(theme.title)} · TEST TERMINÉ</div><h1 tabindex="-1">${score >= 80 ? 'Bien joué !' : score >= 60 ? 'Ça prend forme.' : 'On reprend les repères.'}</h1><div class="rev-score" style="--score:${score}%"><span>${score}<small>%</small></span></div><p><strong>${run.correct} / ${run.total}</strong> bonnes réponses · ${picks.filter(p => p === null).length} passée(s)</p><p class="rev-save-status" role="status">${saving ? 'Enregistrement…' : unsaved.has(run.id) ? 'Résultat non sauvegardé sur cet appareil.' : '✓ Résultat enregistré sur cet appareil.'}</p></div>
      ...
      <div class="rev-result-actions"><button id="rev-again" class="rev-primary" ${saving ? 'disabled' : ''}>Nouveau test · 10 questions ↻</button><button id="rev-reread" class="rev-secondary" ${saving ? 'disabled' : ''}>Relire les fiches</button><button id="rev-export" class="rev-secondary">Exporter mes résultats</button></div>
    ...
    document.querySelectorAll<HTMLButtonElement>('[data-review]').forEach(b => b.onclick = () => lesson(Number(b.dataset.review)));
  }
  return {
    async open() {
      study.stop(); themeOpen = false; active = false; testInProgress = false; lessonIndex = null;
    ...
    leave() { generation++; viewGeneration++; active = false; themeOpen = false; testInProgress = false; study.stop(); },
    hasUnfinished() { return testInProgress || unsaved.size > 0 || study.unsaved(); },
    headerNav: () => themeOpen ? `<div class="rev-header-tabs" role="group" aria-label="Fiches et test : ${esc(theme.title)}"><span class="rev-header-caption">Fiches</span>${theme.pages.map((p, i) => `<button type="button" data-rev-page="${i}" aria-label="Fiche ${i + 1} : ${esc(p.title)}" title="${esc(p.title)}" ${lessonIndex === i ? 'aria-current="page"' : ''} ${saving ? 'disabled' : ''}>${i + 1}</button>`).join('')}<button type="button" id="rev-header-test" aria-label="${testInProgress ? 'Reprendre le test' : 'Lancer le test de 10 questions'}" title="${testInProgress ? 'Reprendre le test' : 'Lancer le test de 10 questions'}" ${active ? 'aria-current="page"' : ''} ${saving ? 'disabled' : ''}>Q</button></div>` : '',
    header: () => `${study.header()}${active ? `<button class="language-button" id="rev-reveal" type="button" aria-label="Afficher les traductions françaises" title="Afficher les traductions françaises" aria-pressed="${frenchVisible}" ${frenchVisible ? 'disabled' : ''}><span class="france-flag" aria-hidden="true">🇫🇷</span></button>` : ''}`,
