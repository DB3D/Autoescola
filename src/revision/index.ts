...
import { drawTest, finishRun, mastery, recentRuns, runPercent, shuffle, TEST_LENGTH } from './engine';
...
export function createRevision(shell: (html: string, testing: boolean) => void) {
  ...
  function history() {
    ...
    return `<div class="rev-history"><div><b>Maîtrise du thème</b><strong>${percent(mastery(runs, theme.id))}</strong></div><p>Moyenne des ${recent.length || 'futurs'} derniers tests terminés${recent.length < 5 && recent.length ? ' · 5 maximum' : ''}.</p><ol>${recent.length ? [...recent].reverse().map(r => `<li><b>${runPercent(r)} %</b><time datetime="${r.completedAt}">${esc(date(r.completedAt))}</time></li>`).join('') : '<li>Pas encore évalué. Le premier test donnera ton point de départ.</li>'}</ol><p class="rev-theme-time">⏱ Temps total sur ce thème : <strong data-study-theme="${theme.id}">${studyDuration(themeSeconds(theme.id))}</strong><br>Lecture + tests + corrections</p></div>`;
  }
  function sources() {
  ...
  function lesson(pageIndex: number) {
    ...
    render(`<div class="rev-lesson rev-${theme.color}"><div class="rev-eyebrow">${theme.icon} ${esc(theme.title)} · Fiche ${pageIndex + 1} / ${theme.pages.length}</div><h1 tabindex="-1">${esc(p.title)}</h1><p class="rev-kicker">${esc(p.kicker)}</p>
  ...
  function question() {
    ...
    render(`<div class="rev-test-top"><span>Question <b>${index + 1} / ${TEST_LENGTH}</b></span></div><div class="rev-progress" role="progressbar" aria-label="Questions terminées" aria-valuenow="${index}" aria-valuemin="0" aria-valuemax="${TEST_LENGTH}"><span style="width:${index * 10}%"></span></div>
  ...
  return {
    ...
    headerLead: () => themeOpen ? `<button type="button" class="rev-header-back" id="rev-home" aria-label="Retour aux thèmes" ${saving ? 'disabled' : ''}><span aria-hidden="true">←</span> Thèmes</button>` : '',
    headerNav: () => themeOpen && !active ? `<div class="rev-header-tabs" role="group" aria-label="Fiches et test : ${esc(theme.title)}"><span class="rev-header-caption">Fiches</span>${theme.pages.map((p, i) => `<button type="button" data-rev-page="${i}" aria-label="Fiche ${i + 1} : ${esc(p.title)}" title="${esc(p.title)}" ${lessonIndex === i ? 'aria-current="page"' : ''} ${saving ? 'disabled' : ''}>${i + 1}</button>`).join('')}<button type="button" id="rev-header-test" aria-label="Lancer le test de 10 questions" title="Lancer le test de 10 questions" ${saving ? 'disabled' : ''}>Q</button></div>` : '',
