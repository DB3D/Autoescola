...
export function createRevision(shell: (html: string, testing: boolean) => void) {
  ...
  function lesson(pageIndex: number) {
    ...
    render(`${back}<div class="rev-lesson rev-${theme.color}"><div class="rev-eyebrow">${theme.icon} ${esc(theme.title)}</div><div class="rev-page-tabs" aria-label="Fiches du thème">${theme.pages.map((_, i) => `<button data-page="${i}" ${i === pageIndex ? 'aria-current="step"' : ''}>Fiche ${i + 1}<span>${i === pageIndex ? '●' : '○'}</span></button>`).join('')}</div><h1 tabindex="-1">${esc(p.title)}</h1><p class="rev-kicker">${esc(p.kicker)}</p>
      ...
      <div class="rev-ready"><span>${pageIndex === 0 ? 'Une fiche de plus, puis à toi de jouer.' : 'Tu te sens prêt ? À toi de jouer.'}</span><button class="rev-primary" id="rev-continue">${pageIndex === 0 ? 'Lire la fiche 2 →' : 'Lancer le test · 10 questions →'}</button><p>${pageIndex === 0 ? 'Tu peux relire les fiches à ton rythme.' : '5 questions par fiche · tirage aléatoire · en catalan'}</p></div>${history()}${sources()}</div>`);
    button('rev-home', overview);
    document.querySelectorAll<HTMLButtonElement>('[data-page]').forEach(b => b.onclick = () => lesson(Number(b.dataset.page)));
  ...
  function question() {
    ...
    render(`<div class="rev-test-top"><button id="rev-quit" class="rev-back">← Quitter</button><span>Question <b>${index + 1} / ${TEST_LENGTH}</b></span></div><div class="rev-progress" role="progressbar" aria-label="Questions terminées" aria-valuenow="${index}" aria-valuemin="0" aria-valuemax="${TEST_LENGTH}"><span style="width:${index * 10}%"></span></div>
      <div class="rev-eyebrow">${theme.icon} ${esc(theme.title)}</div><p class="rev-test-hint">Une seule réponse · correction à la fin</p><h1 class="rev-question" lang="ca" tabindex="-1">${esc(q.prompt)}</h1>
      <div class="rev-answers" lang="ca" role="group" aria-label="Respostes">${order.map((original, i) => `<button data-choice="${original}" aria-pressed="${selected === original}"><span>${String.fromCharCode(65 + i)}</span>${esc(q.answers[original])}</button>`).join('')}</div>
      <div class="rev-test-actions"><button id="rev-next" class="rev-primary" ${selected === undefined ? 'disabled' : ''}>${index === TEST_LENGTH - 1 ? 'Voir mon résultat' : 'Question suivante →'}</button><button id="rev-skip" class="rev-secondary" lang="ca">No ho sé · passar</button></div><div id="rev-quit-dialog"></div>`, true);
    document.querySelectorAll<HTMLButtonElement>('[data-choice]').forEach(b => b.onclick = () => {
      selected = Number(b.dataset.choice);
      document.querySelectorAll('[data-choice]').forEach(el => el.setAttribute('aria-pressed', String((el as HTMLElement).dataset.choice === String(selected))));
      document.querySelector<HTMLButtonElement>('#rev-next')!.disabled = false;
    });
    button('rev-next', () => { if (selected !== undefined) advance(selected); });
    button('rev-skip', () => advance(null));
    button('rev-quit', () => {
      ...
      button('rev-abandon', () => { active = false; questions = []; lesson(0); });
    });
  }
  function advance(pick: number | null) {
