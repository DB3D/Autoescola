...
Revision tests also show the French flag beside their study clock. Question and answer translations start blurred; the flag reveals them, or answering reveals them automatically alongside immediate correct/incorrect feedback. A separate click in the content advances to the next question (or the final result), without a Next button. Revision has no French assistance or time penalty.

Inside a Revision theme, a second row in the sticky header contains lesson numbers and a Q button on the right. The numbers open the lessons; Q starts or resumes the theme test. Reading during a test preserves its current question, answers, feedback and French visibility. All 28 lessons use expanded French explanations with context and examples, while keeping numbered visual blocks, vocabulary, memory aids and Catalan grammar boxes.
